...
import { SeriesPost, StringObject } from '../../../types'
 ...
 */
export const syncGists = run(async (event, _context) => {
  const params = qs.parse(event.body)
  const [series, gists] = await fetchGists(new Date(params.since as string || new Date()))
  ...
  for (const gist of gists) {
    ...
    if (!record || record.updatedAt !== gist.updatedAt || record.rawUrl !== gist.url) {
      ...
      record.createdAt = gist.createdAt!
      record.updatedAt = gist.updatedAt!
